...
// 故前端硬编码展示 macOS 预期路径 + 说明文案；T9 可补 get_db_path 命令。
// 「在 Finder 中显示」：Tauri v2 shell 插件需注册，v1.0.0 未注册，按钮 alert 提示「v1.1+ 完善」，
// 不为此加 src-tauri 改动。
  ...
  return (
    <Card title="数据库路径" size="small">
      <Space direction="vertical" style={{ width: "100%" }}>
        ...
        <Text type="secondary">
          这是 v1.0.0 在 macOS 上的预期数据库位置；T9+ 将提供动态获取命令。
        </Text>
        <Button
          icon={<FolderOpenOutlined />}
          onClick={() =>
            message.info("「在 Finder 中显示」v1.1+ 完善")
          }
        >
          在 Finder 中显示
        </Button>
