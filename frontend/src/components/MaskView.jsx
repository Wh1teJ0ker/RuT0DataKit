...
import { applyRulesCols, listMaskOpTypes } from "../tauri.js";
...
export default function MaskView({ state, dispatch }) {
  ...
  const resolveMask = (header) => {
    ...
    return (
      state.rules.maskers.find((r) => r.field === header) || null
    );
  };
  ...
  }, [state.maskedRows, state.headers]);
  ...
  const effectiveMaskers = useMemo(() => {
    const map = new Map();
    // 全局规则库兜底
    for (const r of state.rules.maskers) {
      map.set(r.field, r);
    }
    // 会话级 override 覆盖
  ...
  }, [state.rules.maskers, state.maskOverrides]);
  ...
  const onApply = async () => {
    ...
    try {
      ...
      const r = await applyRulesCols(state.filePath, rulesJson, selectedColumns);
      dispatch({
        type: "SET_MASKED",
