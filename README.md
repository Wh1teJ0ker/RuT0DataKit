...
> **当前状态：v0.1.0 UI 重构 + 规则引擎重构补丁 + 双 state 分离 / 边界 BUG 修复（T0-21~T0-26 + 后续修复补丁）已完成。** v0.1.0 交付四功能导航（数据脱敏 / 数据校验 / 数据导出 / 规则管理）+ 抽象算子规则引擎（MaskOp/ValidateOp + 预置库 + 试运行 API）+ 规则管理单一列表 + Drawer + 动态试运行 + 数据脱敏/校验四段垂直 view + 数据导出单一 Table 集成 + 按列勾选脱敏 + 数据校验 GUI + 数据导出列选/列序/CSV+XLSX + 跨视图 state 持久化 + 规则/映射双 state 分离（MaskView/ValidateView 临时映射不污染全局规则库）+ `MaskOp::Template` 边界 BUG 修复（`keep_prefix + keep_suffix == n` 时正确输出纯脱敏段，不再错拼原值）。版本状态约定见 `docs/04-版本标准.md`。
...
| v0.1.1（规划） | 日志文件解析（按正则提取 mac / username / id_card / phone 等敏感字段） | 规划中 |
| v0.1.2（规划） | pcap 流量包敏感数据提取（依赖系统 tshark） | 规划中 |
...
- **tshark**：仅 **v0.1.2**（pcap 解析）需要。v0.1.0 与 v0.1.1 **不需要** tshark。
...
| `bankcard` | 银行卡号格式校验 |
| `email` | 邮箱格式校验 |
| `mac` | MAC 地址格式校验（v0.1.1 日志扫描用） |
| `username` | 用户名格式校验（v0.1.1 日志扫描用） |
| `name` | 姓名格式校验 |
...
v0.1.0 GUI 已通过「数据校验」view 暴露校验入口：选字段 + validator → 运行校验 → 非法单元格高亮 → 可跳转「数据导出」导出合法/非法子集；validator 同时供 v0.1.1 日志扫描 pipeline 复用。
...
| v0.1.1 | 日志文件解析（正则扫描敏感字段） | 规划中 |
| v0.1.2 | pcap 流量包敏感数据提取（依赖 tshark） | 规划中 |
...
- pcap 解析（v0.1.2）依赖系统 `tshark`，仍在本机执行。
