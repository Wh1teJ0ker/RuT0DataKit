...
> **Current status: v0.7.3 released (T27-1 ~ T27-3 verified_complete, release_complete).** v0.7.3 is a patch release following v0.7.2 that adds a one-click blind-probe auto-extract button to PreprocessView:
> - **PreprocessView adds a "Blind Auto-Extract" button + detect_sql_blind_features now returns body_size/source_ip (T27-1 ~ T27-2)**: user reported they could not manually obtain data prefixed with body_size like `862|username=1'%20or%20ascii(substr((database()),1,1))%3E110%23&password=1`. Root cause: the log source's `size` column IS the HTTP response body byte count and the `ip` column IS the source IP (both already present in records), but `detect_sql_blind_features` **discarded** body_size and source_ip when scanning, returning only `samples: Vec<String>` (pure SQL text). Fix: extended the command to return `samples: Vec<{ sql, body_size, source_ip }>` — locates the `size` column + `ip` column by header name from the same row, deduplicates by the `(sql, body_size, source_ip)` triple; PreprocessView's jump section gains a "Blind Auto-Extract" button (`handleBlindAutoExtract`) that scans the whole table → builds `body|sql` text → calls `parseSqlTool` → jumps to Tools/Sql. Non-log sources lacking `size`/`ip` columns get body_size=null and fall through to `parse_payload`. The return-shape change is zero-break (no frontend caller existed after v0.5.x removed the auto-jump entry). Added 4 Tauri unit tests.
> - **Version bump 0.7.2 → 0.7.3**: four manifests synced (Cargo.toml workspace / src-tauri Cargo.toml / tauri.conf.json / frontend package.json) + Cargo.lock ×2. Non-breaking patch: extends an existing Tauri command's return shape + frontend button calling the existing `parseSqlTool`; `SqlParseInput` struct + `parse_sqls` signature unchanged. Version-status convention see `docs/04-版本标准.md`.
> - See `docs/versions/0.7.3/更新日志.md` and `docs/qa/versions/0.7.3/QA-审计报告.md`.
