...
> **Current status: v0.7.1 released (T25-1 ~ T25-2 verified_complete, release_complete).** v0.7.1 is a patch release following v0.7.0 that adds URL-encoded input recognition to the SQL-parsing path:
> - **SQL-parsing path auto-recognizes URL-encoded input (T25-1)**: user reported that URL-encoded SQLi payloads (e.g. `username=1'%20or%20ascii(substr((database()),1,1))%3E79%23&password=1`) failed to parse. Root cause: `parse_sqls` / `detect_sql_blind_features` fed raw input directly to plain-text regexes expecting literal space / `>` / `#`; `%20` / `%3E` / `%23` did not match → 0 probes. Fix: new `crates/core/src/log/mod.rs::looks_like_url_encoded` (regex `(?i)%[0-9a-f]{2}` + `OnceLock` singleton); `parse_sqls` and `detect_sql_blind_features` decode via the existing `url_decode_twice` only when matched, then feed the probe regex. Callers may pass raw input directly; already-decoded input is unaffected (backward compatible). Added 8 core + 3 Tauri unit tests.
> - **Version bump 0.7.0 → 0.7.1**: four manifests synced (Cargo.toml workspace / src-tauri Cargo.toml / tauri.conf.json / frontend package.json). Non-breaking patch: only relaxes the `SqlParseInput.sql` field semantics (callers may pass raw); all other public APIs / behaviors are unchanged. Version-status convention see `docs/04-版本标准.md`.
> - See `docs/versions/0.7.1/更新日志.md` and `docs/qa/versions/0.7.1/QA-审计报告.md`.
