...
> - **ToolsView dropdown (T6-3)**: `ToolsView` switches from antd `Tabs` (horizontal tabs) to antd `Select` (dropdown), `options=[{value:"sql",label:"SQL 解析"},{value:"regex",label:"正则解析"}]`, `onChange` dispatches `SET_TOOLS_ACTIVE_TAB`, default sql fallback.
...
- **T6-3 ToolsView dropdown**: `ToolsView` switches from antd `Tabs` (horizontal tabs) to antd `Select` (dropdown), `options=[{value:"sql",label:"SQL 解析"},{value:"regex",label:"正则解析"}]`, `onChange` dispatches `SET_TOOLS_ACTIVE_TAB`, default sql fallback; user feedback was that horizontal tabs take up space, the dropdown is more compact.
