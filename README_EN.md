...
> **Current status: v0.4.3 txt compatibility + data-extraction module + rule-engine de-absolutization (T9-1 ~ T9-7 verified_complete, release_complete).** v0.4.3 adds txt compatibility, a standalone "Data Extraction" module, and rule-engine de-absolutization on top of v0.4.2's settings module + patch fixes:
> - **txt compatibility (T9-1)**: core adds `crates/core/src/readers/txt_reader.rs` (mirrors SqlReader — whole-text → `Records { headers: ["content"], rows: [[entire-text]] }`); `pipeline/mod.rs::SourceType` gets `Txt` + `detect_type("*.txt") => Txt`; `readers/mod.rs` dispatch arm; PreprocessView file-dialog filter received `.txt` in T9-4, so .txt files can be imported and previewed.
> - **Rule-engine de-absolutization (T9-2)**: `validators/phone.rs` deletes CTF_PREFIXES / REAL_PREFIXES whitelists + ctf_prefixes()/real_prefixes() + OnceLock/HashSet, validating only by PDF spec `^1\d{10}$` (the old "155..." non-whitelisted segment used to fail; now valid=true); new `validators/ip.rs::IpValidator` (uses `IP_REGEX`) registered as the 8th built-in validator; `scan/mod.rs::extract_pattern` gains an `"ip"` entry so the sensitive scanner can also extract IP candidates from text and validate them.
> - **Data-extraction module (T9-3 + T9-4 + T9-5)**: core adds `crates/core/src/extract/mod.rs` (`builtin_extract_ruleset` with three validators + `extract_text(content)` / `extract_file(path)` reusing `DefaultSensitiveScan`); Tauri gets three new commands `extract_text` / `extract_file` / `export_extract` (reusing `write_csv` / `write_json` + type_value concatenation); frontend `ExtractView.jsx` (NEW) with a 4-Card layout — input (Radio switch file/text) + action (start extraction) + results (Table + count Tag) + export (txt/csv/json buttons); Sidebar gets a new `{ key: "extract", icon: <FilterOutlined />, label: "数据提取" }` entry after export.
> - **Version bump 0.4.2 → 0.4.3 (T9-4)**: four places synced — `src-tauri/Cargo.toml` + `tauri.conf.json` + `frontend/package.json` + README status line.
> Safety constraint preserved: all-local processing; extract commands make no network calls; rules and samples are never uploaded; zero new Cargo / npm dependencies (reuses antd / @ant-design/icons / DefaultSensitiveScan / ValidatorRegistry / write_csv / write_json). See `docs/04-版本标准.md` for the version status convention.
