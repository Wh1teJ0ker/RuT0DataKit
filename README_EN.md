...
> **Current status: v0.4.1 five-issue defect fix (T6-1 ~ T6-6 all verified_complete, release_complete).** v0.4.1 targets five user-reported problems on top of v0.4.0's 7-view architectural refactor:
> - **Data-flow wired end-to-end (T6-1)**: `ExportView` migrates from reading `state.filePath` (the SET_FILE channel, never actually written in v0.4.0) to reading `state.records` (the SET_RECORDS channel, aligned with `PreprocessView.handleImport` as the sole writer); `computeExportArgs` falls back to records as the backend inputPath; new e2e `preprocess_to_search_finds_hits` (csv → `read_records` → `search_records(Keyword "张三")` asserts hit count ≥1 and the matched cell contains "张三") verifies the "preprocess → search" link end-to-end.
> - **Remove FileToolbar from every view (T6-2)**: delete `frontend/src/components/FileToolbar.jsx` (-106 lines) + `App.jsx` drops the `NO_TOOLBAR_VIEWS` set and `<FileToolbar/>` render branch; the only import entry point now converges on `PreprocessView`'s built-in "Select File" button (the v0.4.0 design intent).
> - **ToolsView dropdown (T6-3)**: The "Tools" entry in `Sidebar.jsx` changes from a plain Menu item to an antd `Menu.SubMenu`, `children=[{key:"tools.sql",label:"SQL 解析"},{key:"tools.regex",label:"正则解析"}]`; clicking the "Tools" title expands/collapses (`openKeys` controlled as `state.sidebarToolsOpen`); clicking a sub-item dispatches both `SET_VIEW("tools")` and `SET_TOOLS_ACTIVE_TAB(<sql|regex>)`; `ToolsView.jsx` itself no longer renders an in-view `Select` dropdown (semantically duplicates the Sidebar SubMenu) and only renders the Card + sub-tool content, with the Card `title` switching with `toolsActiveTab`.
> - **SQL blind-injection feature auto-jump (T6-4)**: core `looks_like_blind_probe(sql: &str) -> bool` reuses `ascii_binary_regex` / `equality_regex` / `length_regex` and does not depend on `response_body_size` (unlike `extract_blind_probe`, so it works for preprocessing where only SQL text is available); Tauri `detect_sql_blind_features(headers, rows) -> {detected, samples}`; GUI `PreprocessView.handleImport` auto-jumps to SqlParseTool and prefills samples on detection; pure local regex matching, no data exfiltration.
> - **RegexTool statement → constructed regex (T6-5)**: drops the built-in template Tab and adds a `ConstructTab` (antd `TextArea` statement → `regexConstruct(statement)` → `pattern` `Paragraph` copyable + `matched_clues` `Tag` list + sample highlight); core gains `crates/core/src/tools/regex_construct.rs::construct_regex(statement) -> Result<ConstructedRegex, CoreError>` with rule-based inference over 6 clue types (digit-count / charset / anchoring prefix / email / URL / id-card), semantic priority email > URL > id-card > generic, ending with a `Regex::new` validation so the pattern always compiles; 10 unit tests green.
> Safety constraint preserved: all-local processing, rules and samples are never uploaded. See `docs/04-版本标准.md` for the version status convention.
