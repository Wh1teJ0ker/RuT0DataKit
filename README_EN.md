...
> **Current status: v0.7.0 released (T24-1 ~ T24-4 verified_complete, release_complete).** v0.7.0 is a "tool consolidation + column-level SQL parsing" functional release for data-security CTF scenarios:
> - **Delete Tools/Regex-parsing sub-tool (T24-1)**: user explicitly requested removal of the regex-parsing feature. Frontend `RegexTool.jsx` / `RegexConstructTab.jsx` deleted; `ToolsView.jsx` / `Sidebar.jsx` / `state.js` / `tauri.js` cleaned up in sync; Tauri `commands/tools.rs` rewritten to expose only `parse_sql_tool`, `main.rs` drops the `explain_regex` / `regex_construct` registrations (handler count 39 → 37); core `tools/mod.rs` rewritten to expose only `encrypt` + `sql_parse`, files `regex_explain.rs` / `regex_construct.rs` / `regex_template.rs` deleted. **Preserved**: `SearchQuery::Regex`, masker-internal regex, and logsign blind regex (unrelated to this sub-tool).
> - **PreprocessView column-level SQL-parse jump (T24-2)**: a `ConsoleSqlOutlined` button is added to the preview-table header next to the ✏ rename button; clicking it triggers `handleColumnSqlParse(columnName)` — collects all non-empty cell values of that column as `SqlParseInput[]` → `parseSqlTool(inputs)` → writes `SET_SQL_PARSE_INPUT` + `SET_SQL_PARSE_RESULT` → jumps to Tools/Sql reusing the existing `SqlParseTool.jsx` UI (mirroring the Sidebar.jsx:60-62 jump pattern). The user can keep editing the textarea in Tools/Sql to rerun.
> - **Version bump 0.6.8 → 0.7.0**: four manifests synced (Cargo.toml workspace / src-tauri Cargo.toml / tauri.conf.json / frontend package.json). Breaking change: the Tools/Regex-parsing sub-tool is removed (per explicit user request); all other public APIs / behaviors are unchanged. Version-status convention see `docs/04-版本标准.md`.
> - See `docs/versions/0.7.0/更新日志.md` and `docs/qa/versions/0.7.0/QA-审计报告.md`.
