...
> **Current status: v0.4.2 settings module first installment (T7-1 ~ T7-4 verified_complete, release_complete).** v0.4.2 adds a sidebar-bottom "设置" entry and tshark multi-platform auto-detection + path configuration on top of v0.4.1's five-issue defect fix:
> - **tshark multi-platform auto-detection (T7-1)**: core adds `crates/core/src/pcap/detect.rs` — a process-level global override path (`Mutex<Option<String>>`) plus `set_tshark_path` / `get_tshark_path` / `resolve_tshark_cmd` / `detect_tshark` / `candidate_paths`. `detect_tshark` probes by priority override → PATH `tshark` → per-platform candidate absolute paths (macOS homebrew / Wireshark.app / Linux `/usr/bin` / Windows Program Files), running `<path> --version` and treating exit 0 as usable, returning `TsharkInfo { path, version }`. `PcapReader::read`'s two `Command::new("tshark")` calls switch to `Command::new(&resolve_tshark_cmd())` — when the override is None the behavior is identical to v0.4.1 (zero regression).
> - **Tauri commands + persistence (T7-2)**: three new commands in `src-tauri/src/commands.rs` — `detect_tshark() -> {path, version}`, `load_tshark_path(app) -> Option<String>` (reads `app_config_dir/settings.json` and injects the saved path into core at runtime), `save_tshark_path(app, path: Option<String>) -> ()` (`create_dir_all` + pretty-printed JSON write + runtime injection). `settings.json` schema: `{ "tshark_path": "/opt/homebrew/bin/tshark" }` (single field, extensible). No new Cargo dependencies — reuses `std::sync::Mutex` / `std::fs` / `tauri::Manager::path().app_config_dir()`. Version bumped 0.4.1 → 0.4.2 in four places (`src-tauri/Cargo.toml`, `tauri.conf.json`, README status + feature table).
> - **Sidebar bottom entry + SettingsView UI (T7-3)**: `Sidebar.jsx` — Menu gets `flex: 1` to push a new "设置" `Button block` (with `SlidersOutlined` icon + status Tag: green「tshark ✓」when detected, red「tshark ✗」when not) to the very bottom of the sidebar, separated by a top border. New `frontend/src/components/SettingsView.jsx` — Card + Descriptions + Space layout; on mount auto-calls `loadTsharkPath()` then `detectTshark()`; four action buttons (auto-detect / use detected path / select file… / clear custom path); an Alert lists common per-platform tshark paths for reference. State fields `tsharkPath` / `tsharkDetected` / `tsharkLoading` added to `frontend/src/state.js`; three camelCase wrappers in `frontend/src/tauri.js`; App.jsx routes `view === "settings"` to SettingsView.
> - **Search substring matching fix (T7 incidental)**: a v0.4.1 user report — "searching「张三」finds hits, but searching「张」does not". Root cause: `tokenize` aggregates continuous Chinese into a single token, and the original `search_keyword` did exact match on postings keys, missing partial-CJK queries. Switched to substring matching: `key.contains(term)` is a hit — searching「张」now matches `张三` / `张三丰`, and ASCII queries benefit too (searching「ali」matches `alice`). New CJK + ASCII substring tests cover both paths.
> Safety constraint preserved: all-local processing, rules and samples are never uploaded; tshark probing and path configuration are fully local, settings.json only writes to the local `app_config_dir`. See `docs/04-版本标准.md` for the version status convention.
...
- **Tools (View 7)**: antd Tabs hosts two sub-tools — (a) **SQL parsing**: `parse_sqls(inputs) -> SqlParseResult { probes, aggregated, reconstructed, parsed_payloads }`, `extract_blind_probe(sql, response_body_size, source_ip)` extracts `BlindProbe` (AsciiBinary / Equality / Length), `reconstruct_database` correlates 4 read_target classes to reconstruct the DB schema/tables; non-blind payloads fall back to `parse_payload`. (b) **Regex parsing**: `explain_regex(pattern) -> Vec<RegexTokenDesc>` hand-written char-by-char scan + 8 built-in templates (replaced by statement→regex construction in v0.4.1).
...
- **T6-3 ToolsView dropdown**: The "Tools" entry in `Sidebar.jsx` changes from a plain Menu item to an antd `Menu.SubMenu`, `children=[{key:"tools.sql",label:"SQL 解析"},{key:"tools.regex",label:"正则解析"}]`; clicking the "Tools" title expands/collapses (`openKeys` controlled as `state.sidebarToolsOpen`, collapsed by default); clicking a sub-item dispatches both `SET_VIEW("tools")` and `SET_TOOLS_ACTIVE_TAB(<sql|regex>)`, and `selectedKeys` highlights `tools.<toolsActiveTab>` when `activeView === "tools"`; `ToolsView.jsx` itself no longer renders an in-view `Select` dropdown (semantically duplicates the Sidebar SubMenu) and only renders the Card + sub-tool content, with the Card `title` switching to `Tools - SQL 解析` / `Tools - 正则解析` based on `toolsActiveTab`; the user's "sidebar Tools should have a dropdown" requirement ultimately lands as the Sidebar SubMenu form.
...
- **T6-5 RegexTool statement → constructed regex**: drops the built-in template Tab and adds a `ConstructTab` (antd `TextArea` statement → `regexConstruct(statement)` → `pattern` `Paragraph` copyable + `matched_clues` `Tag` list + sample highlight); core gains `crates/core/src/tools/regex_construct.rs::construct_regex(statement) -> Result<ConstructedRegex, CoreError>` with rule-based inference over 6 clue types (digit-count / charset / anchoring prefix / email / URL / id-card), semantic priority email > URL > id-card > generic, ending with a `Regex::new` validation so the pattern always compiles; 10 unit tests green. User feedback: "not built-in templates — I give a sentence and it auto-constructs a regex for me."
