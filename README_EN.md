...
> **Current status: v0.3.0 pcap traffic-capture sensitive-data extraction slice (T3-1 ~ T3-5 all verified_complete, release_complete).** v0.3.0 adds a pcap processing slice on top of v0.2.4's blind-aggregation database reconstruction:
> - **tshark subprocess + HttpRequest extraction (T3-1)**: `crates/core/src/pcap/reader.rs::PcapReader::read(path)` invokes system `tshark` (`-Y "http.request" -T fields -e frame.number -e ip.src -e ip.dst -e http.request.method -e http.host -e http.request.uri -e http.file_data -e http.user_agent -E separator=\t -E occurrence=f`) → TSV → `HttpRequest { frame_no, src_ip, dst_ip, method, host, uri, body: Option<String>, user_agent }`; `http.file_data` hex string goes through a hand-written `hex_to_bytes` (no `hex` crate) → `String::from_utf8_lossy`. Missing tshark → `CoreError::DependencyMissing("tshark")`, no panic. Fixture `data.pcapng` (9.4MB / 60290 frames / 5000 HTTP POST) yields ≥5000 HttpRequest.
> - **Decoding (T3-2)**: `decoder.rs` provides `decode_url_twice` (independent impl to avoid circular deps) + `try_decode_base64_field` (hand-written b64 decode, charset/length/printable_ratio triple false-positive guard, UTF-8 only, GBK deferred to v0.3.1+) + `extract_decoded_fields` (JSON / form-urlencoded / plain-text triple fallback) + `reassemble_base64` (rule-based fallback, interface preserved). Fixture first POST body decodes all 7 fields correctly: username=chenyong / name=付里夏旋 / idcard=506051200109055743 / phone=74733385248 / address=黑龙江省哈尔滨市通河县三站镇377号159室.
> - **Scanning (T3-3)**: `scanner.rs::PcapScanner::scan` reuses `DefaultSensitiveScan`, assembles `scan_text` (uri double URL-decoded + body decoded field values) per request → sensitive scan → each Finding gets `location=frame:N` + `context={method} {host} -> {src_ip}`; produces `Report { kind: "pcap_scan", summary: { total_requests, sensitive_hits, decoded_fragments, top_src_ips }, findings, extra: Value::Null }`. New `pcap_scan` pipeline thin wrapper; e2e `pcap_scan_full` asserts ≥4000 sensitive hits.
> - **Command + GUI (T4)**: New Tauri command `scan_pcap_file` (single IPC returns `{ entries, report }`); `PcapView.jsx` four-section layout (① import `.pcap/.pcapng` / ② raw HTTP table / ③ summary + top_src_ips Tag / ④ Findings table with type Tag coloring); sidebar pcap item enabled; tshark-missing → `message.error("流量分析需要系统 tshark，请先安装 Wireshark CLI (brew install wireshark)")`.
> Sensitive scan only, no SQLi signatures (fixture has none); `kind="pcap_scan"` is new and backward compatible with v0.2.4. See `docs/04-版本标准.md` for the version status convention.
