...
- 嵌套 JSON 递归展平（`a.b` 列名）
- ~~多字节字符搜索高亮精确边界~~（v1.1.1 已修复：`TextEncoder`/`TextDecoder` 按字节偏移切片，中英文混合安全）
...
- `cargo test --workspace`：72 passed / 0 failed / 0 ignored（含 commands 层 search 21 tests 含行级搜索 6 + columns 7 + undo/redo 5 + DB 39）
- `pnpm --prefix frontend install --frozen-lockfile`：通过
- `pnpm --prefix frontend build`：通过（3079 modules transformed，2.45s）
...
- 搜索高亮：后端返回**字节偏移**（Rust `str::find` / `regex::Match`），前端用 `TextEncoder`/`TextDecoder` 按字节区间还原字符串再包裹 `<mark>`，中英文混合安全（v1.1.1 已修复初版 UTF-16 切片错位问题）
- 搜索态进入「只保留搜索结果」模式（`sheet.searchRows !== null`），DataTable 切到 `searchRows` 渲染并按命中行分页；清除搜索或全局替换后退回普通翻页模式（`CLEAR_SEARCH` 清空 `searchRows`/`searchHits`/`searchState`）
