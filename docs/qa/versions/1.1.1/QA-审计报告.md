...
> 结论：`qa_passed` — 静态审计 + 单元测试（66 passed）+ 前端构建（3079 modules）+ 版本一致性全通过；T29~T35 全部 `verified_complete`，T36 文档收口 + Release QA 完成且无 major/critical 问题。

## 1. 审计维度与结论

| 维度 | 结论 | 说明 |
|---|---|---|
| 功能完整性 | `pass` | T29~T35 单元级全 `verified_complete`；DB 39 tests + commands 层 27 tests（undo/redo 5 + search 15 + columns 7）+ core 40 tests = workspace 66 passed；撤销/重做 + 列操作（JSON 解析 + 列内替换）+ 搜索（关键字/正则/全局替换）+ 搜索索引优化全部落地 |
| 回归与端到端 | `pass` | `cargo fmt --check` / `cargo clippy --workspace -- -D warnings` / `cargo test --workspace`（66 passed / 0 failed / 0 ignored）+ `pnpm --prefix frontend install --frozen-lockfile && pnpm --prefix frontend build`（3079 modules transformed，2.31s）全绿；GUI 端到端交互（E3~E6）属浏览器自动化增量轮次，不阻塞发布 |
...
| 3 | 搜索后端命令（search_cells + replace_all） | `pass` | `src-tauri/src/commands/search.rs`；search_cells 关键字 LIKE + 正则预筛精匹配 + 分页 + 匹配区间；replace_all 单事务 + before/after 快照撤销；15 tests 全过 |
...
| 5 | 前端 IPC + state 扩展 | `pass` | `frontend/src/tauri.js` 7 IPC（undo/redo 不传 sheetId，searchCells colIdx 可 null）+ `state/constants.js` 5 ACTION + `state/reducer.js` 5 case + `state/factory.js` searchHits + createSheetFromParse；APPLY_SEARCH_HITS rowKey 构造（pageInnerIdx=rowIdx-1-pageBase，越界跳过）；pnpm build 通过 |
| 6 | 前端 UI（撤销工具栏 + 搜索栏 + 列操作面板） | `pass` | `DataTable.jsx` 搜索栏 + mark 高亮（#fff48f）+ 全局替换 Modal；`Workbench.jsx` 撤销/重做工具栏（redoStack 本地维护）；`ColumnOpsPanel.jsx` JSON 解析新 Tab + 列内替换；SidePanel+TopToolbar 注册 columnOps；AppContext 5 dispatcher；state.js barrel createSheetFromParse；pnpm build 通过（3079 modules 2.13s）；T34 review 3 minor 非阻塞 |
...
| `cargo test --workspace` | `pass` | 66 passed / 0 failed / 0 ignored（db 39 + commands 27 + core 40，其中 core 与部分 db 重叠计数） |
| `pnpm --prefix frontend install --frozen-lockfile` | `pass` | Lockfile is up to date，Already up to date（366ms） |
| `pnpm --prefix frontend build` | `pass` | 3079 modules transformed，✓ built in 2.31s（chunk >500kB 为 antd 既有警告，非本轮引入） |
...
| E4 搜索高亮 GUI | `info` | 后端单测覆盖 search_cells 关键字/正则/分页；前端 pnpm build 通过；GUI 视觉验收属增量轮次 |
...
| 项 | 状态 | 证据 |
|---|---|---|
| SQL 参数绑定（v1.1.1 新增 8 方法 + 3 命令组） | `pass` | 全部用 `?N` + `params![]` 绑定；grep 无字符串拼接 SQL；list_undoable_operations 的 IN 子句为硬编码常量（`mask`/`replace_in_column`/`replace_all`），无注入风险 |
...
| `info` | 搜索高亮匹配区间按字节偏移切片，多字节字符（中文）边界可能错位 | v1.1.1 已知简化（T34 scope_deviation 记录），留待 v1.2+ 优化 | `info` |
...
3. **搜索**（T29+T31+T33+T34）—— 关键字 LIKE + 正则预筛精匹配 + idx_cells_sheet_col 复合索引 + 服务端分页 + 全局替换快照撤销 + 单元格 `<mark>` 高亮；
4. **测试优化**（T29~T32）—— 首个 commands 层测试套件（undo/redo 5 + search 15 + columns 7 = 27 tests），DB 层扩展至 39 tests，workspace 全量 66 passed。
...
| `cargo test --workspace` | 66 passed / 0 failed / 0 ignored |
| `pnpm --prefix frontend install --frozen-lockfile` | pass（366ms） |
| `pnpm --prefix frontend build` | pass（3079 modules，2.31s） |
| 版本号一致性（6 处） | pass（1.1.1） |
| schema 迁移幂等 | pass（v2→v3 单测覆盖） |

**门禁裁决**：无未修复的 critical/major 问题。3 项 minor（T34 review 标记）均为非阻塞已知简化，已记录留待后续版本优化。GUI 端到端交互验收（E3~E6）属浏览器自动化增量轮次，不阻塞发布。**结论推进至 `qa_passed`**。
...
| `src-tauri/src/commands/search.rs`（新文件） | search_cells + replace_all | `cargo test --workspace` 15 tests passed |
| `src-tauri/src/commands/columns.rs`（新文件） | parse_column_as_json + replace_in_column | `cargo test --workspace` 7 tests passed |
| `src-tauri/src/lib.rs` | generate_handler! 注册 7 新命令 | `cargo check` 绿 |
| `frontend/src/tauri.js` | 7 IPC 封装（undo/redo 不传 sheetId） | `pnpm build` 绿 |
| `frontend/src/state/constants.js` | 5 ACTION + initialState.undoStack + searchState | `pnpm build` 绿 |
| `frontend/src/state/reducer.js` | 5 case（SET_SEARCH_STATE / APPLY_SEARCH_HITS / CLEAR_SEARCH / ADD_SHEET_FROM_PARSE / SET_UNDO_STACK） | `pnpm build` 绿 |
| `frontend/src/state/factory.js` | searchHits 字段 + createSheetFromParse | `pnpm build` 绿 |
| `frontend/src/components/DataTable.jsx` | 搜索栏 + mark 高亮 + 全局替换 Modal | `pnpm build` 绿 |
...
| `frontend/src/state/AppContext.jsx` | 5 dispatcher | `pnpm build` 绿 |
...
| `docs/02-技术设计文档.md` | 版本覆盖说明 + schema v3 + §4.6 mask_column 签名 + §4.7~§4.9 新命令 + §6 state 扩展 + 搜索高亮 + rules.replacement 描述 | 人工核对 |
...
| `docs/versions/1.1.1/更新日志.md` | T29~T36 进度表 + 关键设计决策 | 人工核对 |
| `docs/versions/1.1.1/RELEASE-NOTES.md`（新文件） | 用户面向 + 已知限制 + 升级 | 人工核对 |
